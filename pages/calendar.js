...
import {
  ...
  addNewTask,
  updateTask,
} from "../utils/db/queryFunctions/tasks";
...
export async function getServerSideProps({ req, res }) {
  ...
  await queryClient.prefetchQuery(
    ["tasks"],
    prismaGetAllTasks(session.user.email)
  );
  await queryClient.prefetchQuery(
    ["categories"],
    prismaGetAllCategories(session.user.email)
  );
  await queryClient.prefetchQuery(
    ["settings"],
    prismaGetAllCategories(session.user.email)
  ...
  await queryClient.prefetchQuery(
    ["theme"],
    prismaGetTheme(session.user.email)
  );
...
export default function Calendar() {
  ...
  //TODO Fix query keys to refetch automatically and remove useeffect
  const { data: tasks, isFetching: isFetchingTasks } = useQuery({
    queryKey: ["tasks", session],
    queryFn: () => getAllTasks(session?.user?.email),
  });
  const { data: categories, isFetching: isFetchingCategories } = useQuery({
    queryKey: ["categories", session],
    queryFn: () => getAllCategories(session?.user?.email),
  });
  const { data: prefetchedTheme, isFetching: isFetchingTheme } = useQuery({
    queryKey: ["theme", session],
    queryFn: () => getTheme(session?.user?.email),
  });
  ...
  return (
    <div>
      ...
      <div className="h-screen p-5">
        <TitleBar width={"w-full"} displaySortingMenu={false} />
